...
X_scaled = scaler_ae.fit_transform(X_sequences.reshape(-1, 1)).reshape(
    X_sequences.shape
)
...
_train_torch(autoencoder, X_scaled, X_scaled)
    ],
    verbose=0,
)

# Reconstruct and calculate error
reconstructed = _predict_torch(autoencoder, X_scaled, verbose=0)
...
ax.axvline(
    threshold_ae, color="red", linestyle="--", label="Selected (95th percentile)"
)
...
axes[1].scatter(
    ts.index[anomalies_iso],
    ts.values[anomalies_iso],
    color="red",
    s=50,
    label="Anomalies",
    marker="x",
...
axes[2].scatter(
    ts.index[anomalies_ae_full],
    ts.values[anomalies_ae_full],
    color="orange",
    s=50,
    label="Anomalies",
    marker="x",
...
axes[3].scatter(
    ts.index[anomalies_stat],
    ts.values[anomalies_stat],
    color="green",
    s=50,
    label="Anomalies",
    marker="x",
...
logger.info(
    f"{'Isolation Forest':<20} {anomalies_iso.sum():<15} {anomalies_iso.mean() * 100:<15.2f}"
)
logger.info(
    f"{'Autoencoder':<20} {anomalies_ae_full.sum():<15} {anomalies_ae_full.mean() * 100:<15.2f}"
)
logger.info(
    f"{'Statistical':<20} {anomalies_stat.sum():<15} {anomalies_stat.mean() * 100:<15.2f}"
)
